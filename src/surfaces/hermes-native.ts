...
const INIT_BEGIN = "# tf:init";
const INIT_END = "# /tf:init";
...
function manifest(project: Project, operations: Operation[]): Record<string, unknown> {
...
function pyproject(project: Project): Record<string, unknown> {
...
function initRegion(project: Project, operations: Operation[]): string {
...
def _register_generated(ctx: Any) -> None:
...
def register(ctx: Any) -> None:
    _register_generated(ctx)
    register_native(ctx)
`;
}

function initTemplate(): string {
  return `${INIT_BEGIN}${INIT_END}


# Yours: use the full PluginContext here. This hook runs after Toolfactory's generated tools.
def register_native(ctx: Any) -> None:
    pass
`;
}
...
export const surface: Surface = {
  ...
  plan(project) {
    ...
    return [
      {
        kind: "merge",
        path: `${dir}/plugin.yaml`,
        format: "yaml",
        patch: manifest(project, operations),
      },
      {
        kind: "region",
        path: `${dir}/__init__.py`,
        regions: [{ begin: INIT_BEGIN, end: INIT_END, content: initRegion(project, operations) }],
        template: initTemplate(),
      },
      {
        kind: "merge",
        path: `${HOST_DIR}/pyproject.toml`,
        format: "toml",
        patch: pyproject(project),
      },
