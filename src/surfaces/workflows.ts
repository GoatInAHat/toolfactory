...
import { OVSX_VERSION, VSCE_VERSION, vsixName } from "./vscode-extension.js";
...
function releaseDocument(
  ...
  for (const id of ["vscode-marketplace", "open-vsx"] as const) {
    const row = rows.find((entry) => entry.id === id);
    if (!row) continue;
    const file = `./${RELEASE_ARTIFACT}/${vsixName(project)}`;
    const publish =
