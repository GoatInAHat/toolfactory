...
tool's own kernel: the background worker POSTs stateless MCP to `toolfactory mcp --http` on
...
destructive. Run `toolfactory mcp --http --pair` and paste the `<url>#<token>` it prints into the
