...
| `mcp` | ships the kernel MCP server (stdio by default; `mcp --http [port]` serves the same server over streamable HTTP at `/mcp`, `mcp --http --pair` under the optional pairing token of §5): `dev.toolfactory/inspector.json`, `mcp.json` references | MCP Inspector `--cli --method tools/list` | portable → `native`; else `excluded:mcp-no-host-capabilities` |
...
`relay-auth-v2`, never a third scheme.
