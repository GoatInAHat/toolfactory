...
(author-owned with marker-delimited generated regions) and merge files (JSON/TOML/YAML where
toolfactory owns exactly the keys in its patch). Keyed contribution arrays track generated entries
by stable identity and only own their projected fields. Author entries and extra native fields
survive rebuilds; obsolete generated entries are removed. A full generated file can migrate to
regions only when its previous lock proves the existing bytes are unchanged.
...
only payload is that same launch row), `mcpb` → `mcp` plus `npm` for TypeScript or `pypi` for
Python (its bundle root is respectively the npm tarball or Python source staged by the package
step), `browser-extension` → `mcp` (its background worker POSTs to that same
`mcp --http` endpoint on loopback), and `vscode-extension` → `mcp`, `cli`, and the binding's
runtime registry (`npm` or `pypi`). `buildPlan`
...
| `mcp-registry` | `server.json` (name `io.github.<owner>/<N>`, one `packages[]` entry per selected package registry with `environmentVariables`/`isSecret`, plus an unconditional `oci` entry — `ghcr.io/<owner, lowercased>/<N>:<version>`, `transport: stdio`) a root `Dockerfile` plus an author-extensible `.dockerignore` region (excluding local environments, VCS data and build caches), one multi-stage build per binding, `LABEL io.modelcontextprotocol.server.name` set to `server.json`'s own `name` in the final stage, `ENTRYPOINT` running the kernel's `mcp` entrypoint directly (`dist/toolfactory/mcp.js`, or `python -m <pkg>.toolfactory.mcp`) — stdio only, since the kernel exposes no `--host` flag to bind past loopback (§4.1) | schema-shaped; published by `mcp-publisher` and, for the image, `docker/build-push-action` | metadata only |
| `mcpb` | `hosts/mcpb/manifest.json`, an MCP Bundle manifest. TypeScript retains manifest v0.2 and stages the npm tarball with production dependencies. Python uses MCPB manifest v0.4 `server.type: uv`, with `uv run --directory ${__dirname} python -m <pkg>.toolfactory.mcp`; `packageSteps` first builds and unpacks the canonical PyPI sdist, including project metadata, source, legal files, and built `web/dist`, so the host-managed uv runtime provisions Python and dependencies. Python selection requires the canonical `pypi` surface. Both forms project `user_config`, admissible `tools[]`, and `privacy_policies` from `tool.json`. No `mcpb` entry is emitted in `server.json`, because the registry schema requires a bundle hash unavailable at plan time | `mcpb validate hosts/mcpb/manifest.json` | as `mcp` |
...
| `cargo`, `nuget`, `maven-central`, `rubygems`, `packagist`, `go-module` | native package release assets/jobs from author-owned metadata; `nativePackages` selects directory, identity file and registry name. Versions must match the shared identity, except VCS registries which derive theirs from the final tag | Cargo package, .NET pack, Maven verify, gem build, Composer validate, Go test | package; no operation translation |
| `homebrew`, `winget`, `scoop`, `chocolatey`, `apt`, `rpm` | platform-specific native package jobs and repository/catalog distribution configured by `systemPackages`; see `docs/system-package-releases.md` | native platform tooling | package; no operation translation |
| `vscode-extension` | `hosts/vscode/`, scaffolded from Microsoft `generator-code` TypeScript/esbuild output. Author-owned activation calls the generated registrar; package contributions merge by command/provider id. Portable operations have command palette entries backed by the upstream MCP client and a native MCP server-definition provider. Settings and SecretStorage project configuration. Development launches the checkout; installed extensions launch the versioned npm/PyPI runtime. Full native VS Code APIs remain available in author code | pinned scaffold comparison, TypeScript/ESLint/esbuild, VSCE VSIX packaging, real Extension Development Host | portable → `bridged:vscode-mcp`; host-specific → `excluded:implement-in-hosts` |
...
| With `web` | `src/toolfactory/web.ts`: the generated `web` operation, appended to `operations` by the `mcp` and `cli` templates so `introspect` snapshots it like any authored one; `web/dist` ships in `package.json` `files` | `src/<pkg>/toolfactory/web.py`, appended the same way; Hatch `artifacts` explicitly includes the gitignored compiled `web/dist`, and `only-include` plus `sources` maps it to `<pkg>/web` without making a fresh `uv sync` require a built page |
| Package | `package.json`, `npm` | `pyproject.toml` (hatchling), `uv`; an optional npm package is a dependency-free `uvx --from <pypi>==<version>` launcher, so PyPI remains the runtime authority |
...
| `publish-pypi` | `pypi` | `gate`, `package` | `id-token: write` | downloads `release-assets/pypi/`, which contains only the prebuilt wheel/sdist, and gives it to `pypa/gh-action-pypi-publish`, environment `pypi` |
