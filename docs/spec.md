...
`check` does not fail when it is absent, only when a copy that is present has gone stale.
...
(the page calls the `mcp --http` endpoint only that surface's CLI subcommand serves). `buildPlan`
...
| `package` | always | `gate` | `contents: read` | `packageSteps` → one `release-assets` artifact (`upload-artifact`): the npm tarball, `uv build`'s distributions, the built `hosts/openclaw` tarball, the plugin-bundle zip, the web tarball, `COVERAGE.md` and a freshly computed `coverage.json` |
