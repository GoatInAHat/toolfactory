...
| `agents` (always on) | `AGENTS.md`, a region file: the generated region carries the commands, layout, boundary law, how to install the tool into the running host (OpenClaw `--link`, Hermes `file://` + restart) and the first-party skills and MCP servers for the selected surfaces; the prose around it is the author's. `init` runs `npx skills add` for those skills, best-effort | — | — |
