...
(its whole bundle is one row launching the kernel MCP server). `buildPlan`
...
| `package` | always | `gate` | `contents: read` | `packageSteps` → one `release-assets` artifact (`upload-artifact`): the npm tarball, `uv build`'s distributions, the built `hosts/openclaw` tarball, the `hosts/dsh` bundle tarball, the plugin-bundle zip, the web tarball, `COVERAGE.md` and a freshly computed `coverage.json` |
