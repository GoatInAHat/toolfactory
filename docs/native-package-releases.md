...
Packagist and Go modules are VCS discovery systems. Packagist reads `composer.json` and tags from the
public repository (configure its webhook once). Go's `name` is the module root, publication is a
pushed semantic-version tag and proxy discovery, and consumption is `go get`; retract with a later
`go.mod` containing a `retract` directive. Both currently require package path `.` because the
release workflow creates a repository-root tag. They deliberately create no pre-release publish
job or fabricated archive upload.
